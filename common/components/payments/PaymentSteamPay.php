<?php

namespace common\components\payments;

use common\models\invoice\Deposit;
use Yii;
use yii\base\Component;

class PaymentSteamPay
{

    public function create($amount)
    {
        $model = Deposit::createOperation(Yii::$app->user->id, $amount, Deposit::TYPE_PAYMENT_STEAM_PAY);
        $result = Yii::$app->freeKassaApi->create($amount, 27, null, $model->id);
        if (!empty($result['error'])) {
            throw new \Exception($result['error'], 414);
        }
        $model->payment_id = $result['orderId'];
        $model->save(false);

        return $result['location'];
    }

    public function check($depositId)
    {
        $model = Deposit::findOne($depositId);
        if (!$model) {
            return null;
        }
        if ($model->status !== Deposit::STATUS_WAIT_CONFIRM) {
            return $model->status;
        }
        $result = Yii::$app->freeKassaApi->check($model->payment_id);
        $status = $result['orders'][0]['status'] ?? null;
        if ($status === 1) {
            $model->markSuccessful();
        } elseif ($status === 8 || $status === 9) {
            $model->markCanceled();
        }

        return $model->status;
    }

    public function debugCheck($depositId)
    {
        return 'not support';
    }

}
