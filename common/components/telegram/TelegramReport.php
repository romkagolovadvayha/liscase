...
class TelegramReport
{
    ...
    {
        if (!TelegramNotificationSettings::isEnabled('tgbotPaymentReport')) {
            return [];
        }
    ...
    private function _getUrl($method, $params = [])
    {
        ...
        $token = Yii::$app->settings->get('tgbotPaymentReport_token');
    ...
    {
        $chatId = Yii::$app->settings->get('tgbotPaymentReport_chatId');
        $params = [
            'chat_id'      => $chatId,
            'text'         => $messageText,
            'parse_mode'   => 'Html',
