...
class Kills extends ActiveRecord
{
    ...
    public static function getWeaponsList() {
        return [
            ...
            'knife.bone' => '/uploads/drop/65_3999fbba739496ab1b5c9b57e2b3e68b.png',
            'stonehatchet' => '/uploads/drop/452_38d00757bcbf2a0f05f103a6cbc35b50.png',
    ...
    public static function getKills($server, $user, $statsModels) {
        ...
        for ($i = 0; $i < count($models); $i++) {
            ...
            if (empty($model['name']) && strlen($model['steam_id']) === 17) {
                $_user = User::findBySteamId($model['steam_id'], false, 'kills 3');
                $model['name'] = $_user->username;
            }
            if (empty($model['dead_name']) && strlen($model['dead']) === 17) {
                $_user = User::findBySteamId($model['dead'], false, 'kills 4');
