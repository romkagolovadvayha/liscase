...
class Servers extends \common\components\base\ActiveRecord
{
    ...
    public function computeWipes(\DateTimeInterface $now = null): array
    {
        $tz = new \DateTimeZone(\Yii::$app->timeZone ?: 'UTC');
        $now = $now ? (new \DateTimeImmutable($now->format('Y-m-d H:i:s'), $tz)) : new \DateTimeImmutable('now', $tz);

        $globalTime = '21:00:00';
        $mapTime    = '16:00:00';

        // ближайший глобал (первый четверг текущего или следующего месяца)
        $global = $this->nearestGlobal($now, $globalTime, $tz);

        // «сегодня», если попадает в правила
        $today = null;
        $todayDate = $now->format('Y-m-d');
        $wt = (int)$this->wipe_type;

        // неделя глобала блокирует остальные вайпы
        $blockedWeeks = [];
        $gIsoWeek = $global->format('o-\WW');
        $blockedWeeks[$gIsoWeek] = true;

        if ($wt === 7 || $wt === 14) {
            $dow = (int)$now->format('N'); // 1..7 (Пн..Вс)
            $iso = $now->format('o-\WW');

            if (!isset($blockedWeeks[$iso])) {
                if ($wt === 7 && $dow == 5) { // пятница
                    $today = $todayDate . ' ' . $mapTime;
                } elseif ($wt === 14 && $dow == 5) {
                    // проверить, попадает ли эта пятница в би-викли сетку
                    $firstFri = $this->firstWeekdayOfMonthDT($now->modify('first day of this month'), 5, $tz);
                    $startDT  = new \DateTimeImmutable($firstFri->format('Y-m-d') . ' ' . $mapTime, $tz);
                    if ($startDT->format('o-\WW') === $gIsoWeek) {
                        $startDT = $startDT->modify('+7 days');
                    }
                    // кратность 14 дней
                    $diffDays = (int)$startDT->diff(new \DateTimeImmutable($todayDate . ' ' . $mapTime, $tz))->days;
                    if ($diffDays % 14 === 0) {
                        $today = $todayDate . ' ' . $mapTime;
                    }
                }
            }
            // четверг глобала — тоже «сегодня»
            if ($now->format('Y-m-d') === $global->format('Y-m-d')) {
                $today = $global->format('Y-m-d H:i:s');
            }
        } elseif ($wt === 30) {
            if ($now->format('Y-m-d') === $global->format('Y-m-d')) {
                $today = $global->format('Y-m-d H:i:s');
        ...
        // следующий вайп (после now)
        $next = null;
        // кандидаты: ближайший глобал (если в будущем) и ближайшие пятницы согласно типу
        $candidates = [];
        ...
        if ($wt === 7 || $wt === 14) {
            // с сегодняшнего дня ищем вперёд
            $cursor = new \DateTimeImmutable($now->format('Y-m-d') . ' ' . $mapTime, $tz);
            // начать от ближайшей пятницы на или после сегодня
            $dow = (int)$cursor->format('N');
            $deltaToFri = (5 - $dow);
            if ($deltaToFri < 0) $deltaToFri += 7;
            $cursor = $cursor->modify('+' . $deltaToFri . ' days');

            // би-викли корректируем старт
            if ($wt === 14) {
                $firstFri = $this->firstWeekdayOfMonthDT(new \DateTimeImmutable($cursor->format('Y-m-01 00:00:00'), $tz), 5, $tz);
                $startDT  = new \DateTimeImmutable($firstFri->format('Y-m-d') . ' ' . $mapTime, $tz);
                if ($startDT->format('o-\WW') === $gIsoWeek) {
                    $startDT = $startDT->modify('+7 days');
                }
                // подвинем cursor на ближайшую к старту кратность 14
                while ($cursor < $startDT) $cursor = $cursor->modify('+7 days');
                $diffDays = (int)$startDT->diff($cursor)->days;
                if ($diffDays % 14 !== 0) {
                    $cursor = $cursor->modify('+' . (14 - ($diffDays % 14)) . ' days');
                }
            }

            // соберём несколько ближайших кандидатов, пропуская недели глобала
            for ($i = 0; $i < 6; $i++) {
                $iso = $cursor->format('o-\WW');
                if ($iso !== $gIsoWeek && $cursor > $now) {
                    $candidates[] = $cursor;
                }
                $cursor = $cursor->modify($wt === 7 ? '+7 days' : '+14 days');
            }
        }

        // выберем минимальный > now
        if (!empty($candidates)) {
        ...
        return [
            'today'  => $today,
            'next'   => $next ? $next->format('Y-m-d H:i:s') : null,
            'global' => $global->format('Y-m-d H:i:s'),
        ];
    }

    /** ближайший глобал (первый четверг сегодня/след. месяца) */
    private function nearestGlobal(\DateTimeImmutable $now, $globalTime, \DateTimeZone $tz): \DateTimeImmutable
    {
        $monthStart = new \DateTimeImmutable($now->format('Y-m-01 00:00:00'), $tz);
        $firstThu   = $this->firstWeekdayOfMonthDT($monthStart, 4, $tz);
        $gDT        = new \DateTimeImmutable($firstThu->format('Y-m-d') . ' ' . $globalTime, $tz);
        if ($gDT <= $now) {
            $nextMonth = $monthStart->modify('+1 month');
            $firstThu  = $this->firstWeekdayOfMonthDT($nextMonth, 4, $tz);
            $gDT       = new \DateTimeImmutable($firstThu->format('Y-m-d') . ' ' . $globalTime, $tz);
        }
        return $gDT;
    }

    /** первый N-й день недели месяца (в формате DateTimeImmutable) */
    private function firstWeekdayOfMonthDT(\DateTimeImmutable $monthStart, $weekday, \DateTimeZone $tz): \DateTimeImmutable
    {
