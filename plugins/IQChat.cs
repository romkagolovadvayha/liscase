...
namespace Oxide.Plugins
{
    ...
    class IQChat : RustPlugin
    {
        ...
        {
            List<BasePlayer> playerList = Pool.Get<List<BasePlayer>>();
            playerList = GetPlayerList(player, channel);
            ...
            if (!String.IsNullOrWhiteSpace(config.ReferenceSetting.translationApiSettings.codeLanguagePrimary))
            {
                Action<String> callback = translation =>
                {
                    ...
                    Pool.FreeUnmanaged(ref playerList);
                };
        ...
        private Object OnPlayerChat(BasePlayer player, String message, Chat.ChatChannel channel)
        {
            ...
            {
                if (channel == Chat.ChatChannel.Team && config.ControllerMessages.TurnedFunc.MuteTeamChat)
                {
                    if (Info.MuteInfo.IsMute(MuteType.Chat))
                    {
        ...
        private void UserConnecteionData(BasePlayer player)
        {
            ...
            if (!UserInformation.ContainsKey(player.userID))
            {
                ...
                // else Info.Info.ColorNick = "#ff6380";
                //
        ...
        private String GetClanTag(BasePlayer player)
        {
            ...
            String ClanTag = (String)Clans?.CallHook("GetClanOf", player.userID.Get());
        ...
        private void FixJsonArrays(JToken token)
        {
            ...
            if (token is JObject obj)
            {
                foreach (var property in obj.Properties().ToList())
                {
                    ...
                    {
                        // Заменяем пустой массив на пустой объект для словарей
                        String propName = property.Name;
                        if (propName.Contains("запрещен") || propName.Contains("Запрещен") || 
                            propName.Contains("Список запрещенных"))
                        {
        ...
        private void LoadConfigFromAPI()
        {
            try
            {
                // Получаем IP и порт сервера (как в RustApp.cs и SignStatistics.cs)
                String serverIp = ConVar.Server.ip;
                Int32 serverPort = ConVar.Server.port; // Используем напрямую, как в RustApp.cs
                
                // Если IP не удалось получить, оставляем 0.0.0.0
                // API должен определять сервер по порту
                
                String pluginName = Name; // "IQChat"
                
                // Исправляем порядок параметров: ip=<IP>&port=<PORT>
                ...
                webrequest.Enqueue(apiUrl, null, (code, response) =>
                {
                    if (code == 200 && !String.IsNullOrEmpty(response))
                    {
                        try
                        {
                            ...
                            {
                                // Исправляем массивы, которые должны быть словарями
                                FixJsonArrays(contentToken);
                                ...
                                if (apiConfig != null)
                                {
                                    ...
                                    // Применяем валидацию и значения по умолчанию
        ...
        private void ValidateAndFixConfig()
        {
            ...
            if (config.OtherSetting == null)
                config.OtherSetting = new Configuration.OtherSettings();

            if (config.OtherSetting.renameSteamApiKey == null)
                config.OtherSetting.renameSteamApiKey = "";

            if (config.ControllerMessages == null)
                config.ControllerMessages = new Configuration.ControllerMessage();

            if (config.ControllerMessages.Formatting == null)
                config.ControllerMessages.Formatting = new Configuration.ControllerMessage.FormattingMessage();
            ...
            if (config.ControllerMessages.Formatting.ControllerNickname == null)
                config.ControllerMessages.Formatting.ControllerNickname = new Configuration.ControllerMessage.FormattingMessage.NickController();
            ...
            if (config.ReferenceSetting == null)
                config.ReferenceSetting = new Configuration.ReferenceSettings();

            if (config.ReferenceSetting.playerRanksSettings == null)
                config.ReferenceSetting.playerRanksSettings = new Configuration.ReferenceSettings.PlayerRanks();

            if (config.ReferenceSetting.playerRanksSettings.colorTag == null)
                config.ReferenceSetting.playerRanksSettings.colorTag =
                    "#FF0000, #FF3300, #FF6600, #FF9900, #FFCC00, #FFFF00, #CCFF00, #99FF00, #66FF00, #33FF00";

            if (config.ReferenceSetting.ClansSettings == null)
                config.ReferenceSetting.ClansSettings = new Configuration.ReferenceSettings.Clans();

            if (config.ReferenceSetting.ClansSettings.colorTag == null)
                config.ReferenceSetting.ClansSettings.colorTag =
                    "#FF0000, #FF3300, #FF6600, #FF9900, #FFCC00, #FFFF00, #CCFF00, #99FF00, #66FF00, #33FF00";

            if (config.ReferenceSetting.XLevelsSettings == null)
                config.ReferenceSetting.XLevelsSettings = new Configuration.ReferenceSettings.XLevels();

            if (config.ReferenceSetting.XLevelsSettings.colorTag == null)
                config.ReferenceSetting.XLevelsSettings.colorTag =
                    "#FF0000, #FF3300, #FF6600, #FF9900, #FFCC00, #FFFF00, #CCFF00, #99FF00, #66FF00, #33FF00";

            if (config.ReferenceSetting.skillTreeSettings == null)
                config.ReferenceSetting.skillTreeSettings = new Configuration.ReferenceSettings.SkillTree();
            ...
            else if (config.ReferenceSetting.xPrisonSettings.colorTag == null)
            {
                config.ReferenceSetting.xPrisonSettings.colorTag =
                    "#FF0000, #FF3300, #FF6600, #FF9900, #FFCC00, #FFFF00, #CCFF00, #99FF00, #66FF00, #33FF00";
            }
