...
namespace Oxide.Plugins
{
    ...
    public class SimpleTime : RustPlugin
    {
        ...
        private ulong customIconSteamId = 76561198028953589;
		...
		private void TimeCommand(BasePlayer player, string command, string[] args)
		{
            ...
            string message = string.Format(lang.GetMessage("Time", this, PlayerID), currentTime);
            
            if (customIconSteamId != 0)
            {
                player.SendConsoleCommand("chat.add", 0, 76561199670355029, message);
                return;
            }
