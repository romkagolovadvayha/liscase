...
{ 
    ...
    class StackSizeController : CovalencePlugin
    {
        ...
        private const string VanillaDefaultsFileName = nameof(StackSizeController) + "_vanilla-defaults";
        private const float StackRebuildDebounce = 2f;

        private Configuration _config;
        private Dictionary<string, int> _vanillaDefaults;

        private readonly HashSet<string> _ignoreSet = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
        {
            "water",
            "water.salt",
            "cardtable",
        ...
        private bool _stackRebuildScheduled;
        private bool _configSaveScheduled;
        ...
        private void Init()
        {
            ...
            if (!LoadVanillaDefaultsFromDisk())
            {
        ...
        {
            if (_configSaveScheduled)
            {
                return;
            }

            _configSaveScheduled = true;
            timer.Once(1f, () =>
            {
                _configSaveScheduled = false;
                SaveConfig();
            });
        }
        ...
        private void PopulateIndividualItemStackSize()
        {
            ...
            if (_config.IndividualItemStackSize.Count == 0)
            {
                ...
                _config.IndividualItemStackSize = new Dictionary<string, int>(_vanillaDefaults);
            }
            else
            {
        ...
        private void GenerateVanillaStackSizeFile()
        {
            ...
            Interface.Oxide.DataFileSystem.WriteObject(VanillaDefaultsFileName,
                vanillaStackSizes);
        ...
        private bool LoadVanillaDefaultsFromDisk()
        {
            try
            {
                var data = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<string, int>>(VanillaDefaultsFileName);
                if (data == null || data.Count == 0)
                {
                    return false;
                }

                _vanillaDefaults = data;
                return true;
            }
            catch
            {
        ...
        private void SetVanillaDefaults(int code, string response)
        {
            ...
            Interface.Oxide.DataFileSystem.WriteObject(VanillaDefaultsFileName, _vanillaDefaults);
        ...
        private int GetStackSize(ItemDefinition itemDefinition)
        {
            try
            {
                if (_ignoreSet.Contains(itemDefinition.shortname))
                {
        ...
        private void ScheduleStackRebuild()
        {
            _pendingRebuild = true;
            if (_stackRebuildScheduled)
            {
                return;
            }

            _stackRebuildScheduled = true;
            timer.Once(StackRebuildDebounce, ApplyQueuedStackRebuild);
        }

        private void ApplyQueuedStackRebuild()
        {
