...
namespace Oxide.Plugins 
{
    ...
    class PermissionsManager : RustPlugin
    {
        ...
        List<ulong> MenuOpen = Pool.Get<List<ulong>>(); 
        Dictionary<string, List<string>> PlugsAndPerms = new Dictionary<string, List<string>>();
            ...
            foreach (var perm in AllPerms)
                if (perm.Contains(name, CompareOptions.IgnoreCase) && !(config.BlockList.Split(',').Contains(name, StringComparer.OrdinalIgnoreCase)))
                {
                    if (!PlugsAndPerms.ContainsKey(name))
                        PlugsAndPerms[name] = new List<string>() { perm };
                    else
                        PlugsAndPerms[name].Add(perm);
                }
            PlugsAndPerms = PlugsAndPerms.OrderBy(x => x.Key).ToDictionary(x => x.Key, x => x.Value);
        }
        ...
        void CloseUIs()
        {
            foreach (BasePlayer player in BasePlayer.activePlayerList.Where(player => player?.net?.connection != null && MenuOpen.Contains(player.userID)))
        ...
        void OnServerInitialized()
        {
            ...
            config.BlockList = config.BlockList.ToLower();
            GetPlugs();
            init = true;
        ...
        void Unload()
        {
            CloseUIs();
            SaveData();
            Pool.FreeUnmanaged<ulong>(ref MenuOpen);
        }
        ...
        {
            if (player?.net?.connection != null && MenuOpen.Contains(player.userID))
                DestroyMenu(player, true);
        }
        #endregion
            ...
            foreach (var entry in plugins.GetAll().Where(x => !x.IsCorePlugin).Select(x => x.Name)) 
            {
                foreach (var perm in AllPerms)
                    if (perm.Contains(entry, CompareOptions.IgnoreCase) && !(config.BlockList.Split(',').Contains(entry, StringComparer.OrdinalIgnoreCase)))
                        if (!PlugsAndPerms.ContainsKey(entry))
                            PlugsAndPerms[entry] = new List<string>() { perm };
                        else
                            PlugsAndPerms[entry].Add(perm);
            }
            PlugsAndPerms = PlugsAndPerms.OrderBy(x => x.Key).ToDictionary(x =>x.Key, x => x.Value);
        }
        ...
        void PlugsUI(BasePlayer player, string msg, string group, int page)
        {
            ...
            foreach (var entry in PlugsAndPerms)
            {
                pos1++;
                plugsTotal++; 
                 
                if (pos1 > 0 && pos1 < 21)
                    elements.Add(new CuiButton { Button = { Command = $"PermsList {entry.Key} null null {group} null 1", Color = config.ButtonColour }, RectTransform = { AnchorMin = $"0.1 {(0.89 - (pos1 * 3f) / 100f)}", AnchorMax = $"0.3 {0.91 - (pos1 * 3f) / 100f}" }, Text = { Text = $"{entry.Key}", FontSize = 10, Align = TextAnchor.MiddleCenter } }, mainName);
                if (pos1 > 20 && pos1 < 41)
                    elements.Add(new CuiButton { Button = { Command = $"PermsList {entry.Key} null null {group} null 1", Color = config.ButtonColour }, RectTransform = { AnchorMin = $"0.4 {(0.89 - ((pos1 - 20) * 3f) / 100f)}", AnchorMax = $"0.6 {0.91 - ((pos1 - 20) * 3f) / 100f}" }, Text = { Text = $"{entry.Key}", FontSize = 10, Align = TextAnchor.MiddleCenter } }, mainName);
                if (pos1 > 40 && pos1 < 61)
                    elements.Add(new CuiButton { Button = { Command = $"PermsList {entry.Key} null null {group} null 1", Color = config.ButtonColour }, RectTransform = { AnchorMin = $"0.7 {(0.89 - ((pos1 - 40) * 3f) / 100f)}", AnchorMax = $"0.9 {0.91 - ((pos1 - 40) * 3f) / 100f}" }, Text = { Text = $"{entry.Key}", FontSize = 10, Align = TextAnchor.MiddleCenter } }, mainName);
            }
        ...
        void PMPermsUI(BasePlayer player, string msg, string plugName, string group, int page)
        {
            var path = ActiveAdmins[player.userID];
            var elements = new CuiElementContainer();
            ...
            elements.Add(new CuiButton { Button = { Command = $"PermsList {plugName} grant null {group} all {page}", Color = config.ButtonColour }, RectTransform = { AnchorMin = $"0.5 {(0.89 - (pos1 * 3f) / 100f)}", AnchorMax = $"0.6 {(0.91 - (pos1 * 3f) / 100f)}" }, Text = { Text = lang.GetMessage("GUIAll", this), FontSize = 10, Align = TextAnchor.MiddleCenter } }, mainName);
            elements.Add(new CuiButton { Button = { Command = $"PermsList {plugName} revoke null {group} all {page}", Color = config.ButtonColour }, RectTransform = { AnchorMin = $"0.65 {(0.89 - (pos1 * 3f) / 100f)}", AnchorMax = $"0.75 {(0.91 - (pos1 * 3f) / 100f)}" }, Text = { Text = lang.GetMessage("GUINone", this), FontSize = 10, Align = TextAnchor.MiddleCenter } }, mainName);
            ...
            for (int i = 0; i < PlugsAndPerms[plugName].Count; i++)
            {
                ...
                pos1++;
                permsTotal++;
                string showName = PlugsAndPerms[plugName][i];
                ...
                if (pos1 > 0 && pos1 < 21)
                {
                    ...
                    if ((bool)PermsCheck(player, group, showName)[0])
                        SetButtons(false);
                    List<string> inheritcheck = (List<string>)(PermsCheck(player, group, showName)[1]);
                    if (inheritcheck.Count > 0)
                    {
                        if (path.inheritedcheck == showName)
                        {
                            ...
                            float h1 = 0, h2 = 0;
                            elements1.Add(new CuiButton { Button = { Command = "", Color = config.ButtonColour }, RectTransform = { AnchorMin = "0.3 0.8", AnchorMax = "0.7 0.825" }, Text = { Text = $"{showName}", FontSize = 12, Align = TextAnchor.MiddleCenter }, }, mainName1);
                            ...
                            for (int j = 0; j < inheritcheck.Count; j++)
                            { 
                                h1 = j * 0.022f; 
                                h2 = j * 0.022f;
                                elements1.Add(new CuiLabel { Text = { Text = $"{inheritcheck[j]}", FontSize = 11, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0 {0.7 - h1}", AnchorMax = $"1 {0.75 - h2}" } }, mainName1);
                            }

                            list = true;
                        }
                        elements.Add(new CuiButton { Button = { Command = $"ShowInherited {plugName} null {showName} {group} null {page} {showName}", Color = config.InheritedColour }, RectTransform = { AnchorMin = $"0.8 {(0.89 - (pos1 * 3f) / 100f)}", AnchorMax = $"0.9 {(0.91 - (pos1 * 3f) / 100f)}" }, Text = { Text = lang.GetMessage("GUIInherited", this), FontSize = 10, Align = TextAnchor.MiddleCenter } }, mainName);
                    }
                    ...
                    elements.Add(new CuiButton { Button = { Command = $"PermsList {plugName} grant {showName} {group} null {page}", Color = ButtonColour1 }, RectTransform = { AnchorMin = $"0.5 {(0.89 - (pos1 * 3f) / 100f)}", AnchorMax = $"0.6 {(0.91 - (pos1 * 3f) / 100f)}" }, Text = { Text = lang.GetMessage("GUIGranted", this), FontSize = 10, Align = TextAnchor.MiddleCenter } }, mainName);
                    elements.Add(new CuiButton { Button = { Command = $"PermsList {plugName} revoke {showName} {group} null {page}", Color = ButtonColour2 }, RectTransform = { AnchorMin = $"0.65 {(0.89 - (pos1 * 3f) / 100f)}", AnchorMax = $"0.75 {(0.91 - (pos1 * 3f) / 100f)}" }, Text = { Text = lang.GetMessage("GUIRevoked", this), FontSize = 10, Align = TextAnchor.MiddleCenter } }, mainName);
                }
            }
            ...
            if (permsTotal > (page * 20))
                elements.Add(new CuiButton { Button = { Command = $"PermsList {plugName} null null {group} null {next}", Color = config.ButtonColour }, RectTransform = { AnchorMin = "0.8 0.02", AnchorMax = "0.9 0.04" }, Text = { Text = "->", FontSize = 11, Align = TextAnchor.MiddleCenter } }, mainName);
            if (page > 1)
                elements.Add(new CuiButton { Button = { Command = $"PermsList {plugName} null null {group} null {previous}", Color = config.ButtonColour }, RectTransform = { AnchorMin = "0.1 0.02", AnchorMax = "0.2 0.04" }, Text = { Text = "<-", FontSize = 11, Align = TextAnchor.MiddleCenter } }, mainName);
            CuiHelper.AddUi(player, elements);
        ...
        void ViewGroupsUI(BasePlayer player, string msg, int page)
        {
            ...
            foreach (var group in permission.GetGroups().OrderBy(x => x))
            {
                ...
                if (pos1 > 0 && pos1 < 21)
                {
                    if (permission.UserHasGroup(path.subject.UserIDString, group))
                        SetButtons(false);
        ...
        private void PermsList(ConsoleSystem.Arg arg)
        {
            var path = ActiveAdmins[arg.Player().userID];
            if (arg.Player() == null || arg.Args == null || arg.Args.Length < 6) return;
            int pageNo = Convert.ToInt32(arg.GetString(5));
            ...
            if (arg.GetString(4) == "all")
            {
                if (arg.GetString(2) != null)
                {
                    ...
                    for (int i = 0; i < PlugsAndPerms[plugName].Count; i++)
                    {
                        currentPlug = PlugsAndPerms[plugName][i];

                        if (config.AllPerPage == true && i > (pageNo * 20) - 20 && i < ((pageNo * 20) + 1)) 
                        {
                            if (action == "grant" && group == "false")
                                permission.GrantUserPermission(Pname, currentPlug, null);
                            if (action == "revoke" && group == "false")
                                permission.RevokeUserPermission(Pname, currentPlug);
                            if (action == "grant" && group == "true")
                                permission.GrantGroupPermission(path.subjectGroup, currentPlug, null);
                            if (action == "revoke" && group == "true")
                                permission.RevokeGroupPermission(path.subjectGroup, currentPlug);
                        }
            ...
            DestroyMenu(arg.Player(), false);
            if (group == "false")
                PMPermsUI(arg.Player(), $"{StripSlashes(path.subject.displayName)} - {plugName}", plugName, group, pageNo);
            else
                PMPermsUI(arg.Player(), $"{path.subjectGroup} - {plugName}", plugName, group, pageNo);
            return;
        }
        #endregion
        ...
        void CmdPerms(BasePlayer player, string command, string[] args)
        {
            ...
            {
                if (permission.GetGroups().Contains($"{args[1]}"))
                {
        ...
        void PurgeUnloadedForPlayers()
        {
            ...
            permission.SaveData();
            Pool.FreeUnmanaged<string>(ref perms);
        }
        ...
        void DoLocal(bool save, bool groups)
        {
            ...
            {
                List<string> perms = Pool.Get<List<string>>();
                List<string> players = Pool.Get<List<string>>();
                if (groups)
                {
                ...
                Pool.FreeUnmanaged<string>(ref perms);
                Pool.FreeUnmanaged<string>(ref players);
            }
            else
            {
        ...
        void DoSQL(bool save, bool groups)
        {
            ...
            Sql.Insert(Core.Database.Sql.Builder.Append($"CREATE TABLE IF NOT EXISTS {config.MySQL.tablename} ( ID VARCHAR(17) NOT NULL, Perms TEXT, Players LONGTEXT, Player TINYINT, PRIMARY KEY (ID)) DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;"), Sql_conn);
            ...
            Sql.Query(sqlString, Sql_conn, list =>
            {
                ...
                else if (!save)
                {
                    ...
                    {
                        string[] perms = dict["Perms"].ToString().Split(',');
                        string[] players = dict["Players"].ToString().Split(',');
                        ...
                        else if (perms != null && !groups && Convert.ToInt16(dict["Player"]) == 1)
                            foreach (var perm in perms)
