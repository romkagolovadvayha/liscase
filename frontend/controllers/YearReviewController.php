...
class YearReviewController extends WebController
{
    ...
    public function actionGenerate($userId)
    {
        ...
        $stats = $this->getPlayerYearStats($user);
        ...
        // Создаем изображение
        $image = $this->generateImage($backgroundPath, $stats, $user);
     ...
     * @return array
     */
    private function getPlayerYearStats(User $user)
    {
        ...
        foreach ($statsData as $row) {
            ...
            // Для wipes берем максимальное значение COUNT(DISTINCT wipe) из всех ключей
            // Но лучше сделать отдельный запрос для точности
        }
        ...
        if (!empty($reportedSteamIds)) {
            $bansFromReports = Bans::find()
                ...
                ->andWhere(['IS NOT', 'banned_at', null])
                ->andWhere(['IS', 'unbanned_at', null]) // Только активные баны
                ->count();
        }
     ...
     */
    private function generateImage($backgroundPath, $stats, $user)
    {
        ...
        // Получаем путь к шрифту (будет переопределяться для каждого элемента)
        $baseFontPath = $this->getFontPath('regular');
        ...
        foreach ($textElements as $index => $element) {
            ...
            if ($element['x'] === 'auto' && isset($element['offsetX'])) {
                // Для "Ept" - позиционируем после "ИТОГИ ГОДА"
     ...
     * @throws \Exception
     */
    private function getFontPath($weight = 'regular')
    {
        ...
        $frontendPath = dirname(__DIR__); // Из frontend/controllers получаем frontend/
     ...
     */
    private function addText($image, $text, $x, $y, $fontSize, $color, $fontPath, $useTTF)
    {
