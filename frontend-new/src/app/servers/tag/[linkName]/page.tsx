...
async function getServerTagData(linkName: string) {
  try {
...
export async function generateMetadata({ params }: { params: Promise<{ linkName: string }> }): Promise<Metadata> {
  const { linkName } = await params;
  const data = await getServerTagData(linkName);
  ...
  return {
    title: data.tag.title || data.tag.name,
    description: data.tag.short_description || '',
  };
}

export default async function ServerTagPage({ params }: { params: Promise<{ linkName: string }> }) {
  const { linkName } = await params;
  const data = await getServerTagData(linkName);
  ...
  return <ServerTagClient tag={data.tag} servers={data.servers} />;
}
