...
export async function GET(
  ...
  try {
    ...
    // Проверяем соответствие URL категории
    // Старый формат: /posts/{categoryLinkName}/{categoryLinkNameChild?}/post-{blogLinkName}
    const expectedParentCategory = slug.length > 2 ? slug[0] : null;
    const expectedCategory = slug.length === 1 ? slug[0] : slug.length > 1 ? slug[slug.length - 2] : null;

    if (expectedParentCategory && row.parent_category_link_name !== expectedParentCategory) {
      return NextResponse.json(
        {
          success: false,
          message: 'Категория не совпадает',
        },
        { status: 404 }
      );
    }

    if (expectedCategory && row.category_link_name !== expectedCategory) {
      return NextResponse.json(
        {
          success: false,
          message: 'Категория не совпадает',
