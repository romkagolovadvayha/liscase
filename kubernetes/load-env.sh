...
ENV_FILE="${1:-${SCRIPT_DIR}/k8s.env}"
NAMESPACE="${NAMESPACE:-liscase}"
CONFIGMAP_NAME="liscase-config"

if [ ! -f "$ENV_FILE" ]; then
    echo "❌ Файл $ENV_FILE не найден!"
    echo "Скопируйте ${SCRIPT_DIR}/k8s.env.example в ${SCRIPT_DIR}/k8s.env и заполните своими значениями:"
    echo "  cp ${SCRIPT_DIR}/k8s.env.example ${SCRIPT_DIR}/k8s.env"
    exit 1
fi
