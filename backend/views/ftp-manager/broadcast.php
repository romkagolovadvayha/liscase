...
<div class="ftp-manager-page w-full flex flex-col min-h-0 flex-1">
    <div class="flex items-center justify-between gap-4 flex-wrap p-4 border-b border-[hsl(0_0%_15.3%_/_1)]">
        ...
        <div class="flex items-center gap-2 flex-wrap">
            <label class="text-xs text-gray-400 whitespace-nowrap">Путь на FTP:</label>
    ...
    <?php else: ?>
        <div class="ftp-tabs-wrap border-b border-[hsl(0_0%_15.3%_/_1)] bg-[hsl(0_0%_18%_/_1)] overflow-x-auto">
            <ul class="ftp-tabs flex items-center gap-0 min-h-0 p-0 m-0 list-none" id="ftp-broadcast-tabs">
                <?php foreach ($serversWithFtp as $idx => $s): ?>
                    <li class="ftp-tab-item ftp-broadcast-tab flex items-center border-b-2 border-transparent -mb-px cursor-pointer px-4 py-3 text-sm text-gray-400 hover:text-white hover:bg-[hsl(0_0%_22%_/_1)] <?= $idx === 0 ? 'active' : '' ?>"
                        data-server-id="<?= (int)$s->id ?>">
...
<div class="ftp-modal-overlay fixed inset-0 bg-black/60 z-50 hidden items-center justify-center p-4" id="ftp-broadcast-upload-modal">
    <div class="ftp-modal bg-[hsl(0_0%_20.4%_/_1)] border border-[hsl(0_0%_15.3%_/_1)] rounded-lg shadow-xl w-full max-w-md">
        <div class="flex items-center justify-between p-3 border-b border-[hsl(0_0%_15.3%_/_1)]">
            <span class="text-white font-medium">Загрузить на все серверы</span>
            <button type="button" class="ftp-broadcast-upload-close text-gray-400 hover:text-white text-2xl leading-none">&times;</button>
        </div>
        <form id="ftp-broadcast-upload-form" class="p-4">
            ...
            <div class="mb-3">
                <label class="block text-gray-400 text-xs mb-1">Файл (одинаковое имя на каждом FTP)</label>
...
(function() {
    ...
    function setActiveTab(serverId) {
        ...
        document.querySelectorAll('.ftp-broadcast-tab').forEach(function(t) { t.classList.toggle('active', t.getAttribute('data-server-id') === serverId); });
