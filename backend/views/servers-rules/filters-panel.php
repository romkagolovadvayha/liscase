...
$categories = ArrayHelper::map(ServersRulesCategory::find()->orderBy('name')->all(), 'id', 'name');
?>
<div class="admin-filters-content bg-[hsl(0_0%_20.4%_/_1)] border-l border-[hsl(0_0%_15.3%_/_1)] h-full overflow-y-auto scrollbar-thin">
    ...
    <div class="p-4 flex-1">
        <div class="mb-6">
            ...
            <div class="space-y-3">
                <div>
                    <label for="serversrulessearch-category_id" class="text-xs text-gray-400 mb-1 block">Категория</label>
                    <div class="ds-select-wrapper">
                        <?= $form->field($searchModel, 'category_id', ['options' => ['class' => 'mb-0'], 'template' => '{input}'])->dropDownList(
                            ArrayHelper::merge(['' => 'Все'], $categories),
                ...
                <div>
                    <label for="serversrulessearch-title" class="text-xs text-gray-400 mb-1 block">Заголовок</label>
                    <?= $form->field($searchModel, 'title', ['options' => ['class' => 'mb-0'], 'template' => '{input}'])->textInput([
                        'class' => 'ds-input w-full text-sm', 'placeholder' => 'Поиск по заголовку',
                    ]) ?>
                </div>
            </div>
        </div>
