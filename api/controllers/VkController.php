    ...
    public function actionWebhook()
    {
        try {
            ...
            if ($type === 'message_new') {
                ...
                // Коды имеют длину 20 символов и генерируются случайно (буквы, цифры, подчеркивания и дефисы)
                // Проверяем, является ли текст кодом подтверждения (длина 20 символов и только буквы/цифры/подчеркивания/дефисы)
                // Используем mb_strlen для правильного подсчета символов в UTF-8
                $textLength = mb_strlen($text, 'UTF-8');
                // Проверяем, что код состоит только из букв, цифр, подчеркиваний и дефисов (без пробелов, эмодзи и других символов)
                if ($textLength === 20 && preg_match('/^[a-zA-Z0-9_-]+$/', $text)) {
